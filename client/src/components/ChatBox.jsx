...
const ChatBox = () => {
    ...
    const onSubmit = async (e) => {
      try {
        e.preventDefault()
        if(!user) return toast('Login Send Message')
        setLoading(true)
        const promptCopy = prompt
        setPrompt('')
        setMessages(prev => [...prev, {role: 'user', content: prompt, timestamp: Date.now(), isImages: false}])
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40 '>
      ...
      <div ref={containerRef}  className='flex-1 mb-5 overflow-y-scroll'>
        ...
        {/* Prompt Input Box */}
        <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 item-center ' >
